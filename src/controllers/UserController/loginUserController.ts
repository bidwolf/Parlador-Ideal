...
export default async function login(req: Request, res: Response): Promise<Response> {
  ...
  try {
    ...
    return res.status(200).json({code:200,message:"Authentication done successfully",TOKEN});

  } catch (error) {
    console.error(error)
    return res.status(500).json({code:500,message:"Something was wrong, please try again later."});
  }
}